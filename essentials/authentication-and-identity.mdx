...
(async () => {
    ...
    const browser = await chromium.connectOverCDP(`wss://connect.anchorbrowser.io?api-key=${ANCHOR_API_KEY}`);
...
with sync_playwright() as p:
    ...
    browser = p.chromium.connect_over_cdp(f"wss://connect.anchorbrowser.io?api-key={ANCHOR_API_KEY}")
    # Mount the browser context in Anchor Browser:
