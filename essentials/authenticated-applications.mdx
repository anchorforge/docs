---
title: 'Authenticated Applications (Beta)'
...
<Warning>
  This feature is currently in **Beta**. Contact [**support**](mailto:support@anchorbrowser.io) to enable this feature.
</Warning>
...
<CodeGroup>
```javascript node.js
await client.applications.authFlows.create(app.id, {
...
```python python
client.applications.auth_flows.create(
...
Use the identity ID to create pre-authenticated browser sessions.
...
console.log(session.ws_endpoint);
```
...
<CardGroup cols={3}>
  ...
  <Card title="API Reference" icon="code" href="/api-reference/applications-beta/">
