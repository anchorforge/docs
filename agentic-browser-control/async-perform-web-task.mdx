...
## Overview
For long `perform-web-task` flows, or parallel execution, async execution is available. This allows you to run the agent prompt asynchronously and poll for results later.
...
- **`COMPLETED`**: The workflow has completed. The `result` field contains the task output as a string. For successful executions, this will be a plain string with the task result. For errors, this will be a JSON string like `"{\"error\": \"Agent execution failed\", \"result\": null}"`
...
for workflow_id in workflow_ids:
    ...
    while status == 'RUNNING':
        ...
        if status == 'COMPLETED':
            results.append(status_data['data']['result'])
            break
        elif status == 'FAILED':
            raise Exception(status_data['data']['error'])
