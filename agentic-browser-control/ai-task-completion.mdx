...
const response = await anchorClient.agent.task(
  'Extract the main heading',                     // Required
  {
    taskOptions: {
      ...
      provider: 'openai',                         // For browser-use agent only, openai, gemini, groq, azure, xai, anthropic
...
- **provider** (string): AI provider to use (`openai`, `gemini`, `groq`, `azure`, `xai`, `anthropic`).
