...
- **max_steps** (integer): Maximum number of steps the agent can take (default: 25)
- **secret_values** (object): Secret values to pass to the agent for secure credential handling
- **highlight_elements** (boolean): Highlight elements during task execution for better visibility
...
const response = await anchorClient.post('/v1/tools/perform-web-task', {
  query: {
    sessionId: latestSessionData?.sessionId || '',
  },
  body: {
...
response = anchor_client.post('/v1/tools/perform-web-task', 
  query={
    'sessionId': latest_session_data.session_id if latest_session_data else ''
  },
  body={
    'detect_elements': True,
    'human_intervention': True,
