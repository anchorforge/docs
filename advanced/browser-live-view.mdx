...
To prevent users from interacting with the browser, add the `pointer-events: none;` style to the iframe:
...
### Embedding Browser with Multiple Tabs

To embed the browser with multiple tabs, use the [Get Pages for a Session](/api-reference/sessions/get-pages-for-a-session) API to obtain metadata about the open tabs:
