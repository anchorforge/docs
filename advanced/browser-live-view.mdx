...
To prevent the end user from interacting with the browser embedding, use the `"read_only": true"` when [creating a session](/api-reference/sessions/create-session).
...
To create a browser in headless mode, set the `headless` parameter to `{"active": false}` when [creating a session](/api-reference/browser-sessions/start-browser-session).
