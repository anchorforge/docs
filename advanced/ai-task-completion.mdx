...
Anchor Browser delivers a record 89% Score on the industry-standard benchmark WebVoyager, leveraging [browser-use](https://github.com/gregpr07/browser-use) as a core componenet of the automation capability.
...
**For basic usage**, the [Perform Web Task Tool](/tools-api-reference/ai-tools/perform-web-task) exposes this capability through a single API request.
...
const result = await ai.evaluate('~ Find the last game played by Milwakee in the NBA and return the result')
console.log(result);
...
with sync_playwright() as p:
    ...
    author = ai.evaluate('~ Find an article that discusses the game and return the name of the author')
    print(author)
