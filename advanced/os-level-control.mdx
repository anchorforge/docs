...
```javascript node.js
  ...
  await anchor_client.sessions.mouse.doubleClick(sessionId, {
    x: 500, 
    y: 200
  });

  // Mouse down and up for custom gestures
  await anchor_client.sessions.mouse.down(sessionId, {
    x: 100, 
    y: 100, 
  });

  // Move while holding down (drag)
  await anchor_client.sessions.mouse.move(sessionId, {
    x: 300, 
    y: 300
  });

  // Release
  await anchor_client.sessions.mouse.up(sessionId, {
    x: 300, 
    y: 300
  });
```
```python python
  ...
  # Double-click for text selection
  anchor_client.sessions.mouse.double_click(sessionId, x = 500, y = 200)

  # Mouse down and up for custom gestures
  anchor_client.sessions.mouse.down(sessionId, x = 100, y = 100)

  # Move while holding down (drag)
  anchor_client.sessions.mouse.move(sessionId, x = 300, y = 300)

  # Release
  anchor_client.sessions.mouse.up(sessionId, x = 300, y = 300)
```
</CodeGroup>
...
```javascript node.js
  ...
  // Drag and Drop
  await anchor_client.sessions.dragAndDrop(sessionId, {
    startX: 200,
    startY: 150,
    endX: 600,
    endY: 400,
...
```python python
  ...
  # Drag and Drop
  anchor_client.sessions.drag_and_drop(sessionId,
    start_x = 200,
    start_y = 150,
    end_x = 600,
    end_y = 400
...
```javascript node.js
  ...
  await anchor_client.sessions.keyboard.type(sessionId, {
    text: "Hello, world!",
...
```python python
  ...
  # Type text with optional delay between keystrokes
  anchor_client.sessions.keyboard.type(sessionId,
    text = "Hello, world!",
    delay = 50 # milliseconds between keystrokes
    )
```
...
```javascript node.js
  ...
  anchor_client.sessions.keyboard.shortcut(sessionId, {
...
```python python
  ...
  # Take screenshot of current browser state
  response = anchor_client.sessions.retrieve_screenshot(sessionId)
  print(response)
...
```python python
  ...
  # Get current clipboard content
  response = anchor_client.sessions.clipboard.get(sessionId)
  print(response)
```
...
# Set clipboard content
anchor_client.sessions.clipboard.set(sessionId, text="Content to copy")

# Trigger copy operation (copies selected text)
copy_response = anchor_client.sessions.copy(sessionId)

# Trigger paste operation
anchor_client.sessions.paste(sessionId, text="Text to paste")
```
</CodeGroup>
...
# Navigate to a specific URL (completely OS-level, operates on selected tab)
response = anchor_client.sessions.goto(sessionId, url="https://example.com")
